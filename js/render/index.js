    ...
    <aside class="help-panel" role="dialog" aria-modal="true" aria-label="使用說明">
      ...
      <div class="help-panel__body">
        <section class="help-section">
          ...
          <p class="help-item">• 景點之間可設定<b>交通方式</b>（步行／大眾運輸／開車／<b>✈️ 飛機</b>）與<b>交通時間</b>，時間軸會自動累加計算每站抵達／離開時間，交通段顯示<b>完整通勤時間</b>（出發–抵達）。選「飛機」或「大眾運輸」時可加填<b>交通備註</b>（航班／班次、月台等，例：台灣虎航 IT250 桃園→關西）。</p>
          <p class="help-item">• 拖曳景點卡片左側握把 <b>⠿</b> 可<b>調整順序</b>（僅握把可起拖，圈選名稱等文字不會誤觸拖曳）；每天可設定<b>出發地</b>與<b>當晚住宿</b>。</p>
        ...
        <section class="help-section">
          ...
          <p class="help-item">• <b>🔗 Notion</b>：匯出 Notion 原生格式的 <code>.zip</code>（檔名帶 ID、資料庫雙 CSV、各列子頁、含導覽頁），可直接匯入 Notion 重建行程、住宿、交通、旅遊攻略資料庫與子頁結構。</p>
...
${cloudSection}
      ...
      <div class="help-panel__foot">
        <b>最後更新</b>：交通方式新增 ✈️ 飛機；飛機與大眾運輸皆可加填交通備註（航班／班次、月台等），飛機段不做自動路線估算；交通段改為顯示完整通勤時間（出發–抵達）；桌機拖曳排序改為僅從握把 ⠿ 起拖，圈選景點名稱等文字不再誤觸拖曳；手機版交通列改為可換行避免超出畫面；手機大字體時時間區間改上下堆疊避免左欄擠壓（2026-08-06）
      </div>
    </aside>
  `;
}
