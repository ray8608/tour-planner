    ...
    <aside class="help-panel" role="dialog" aria-modal="true" aria-label="使用說明">
      ...
      <div class="help-panel__body">
        <section class="help-section">
          ...
          <p class="help-item">• 景點之間可設定<b>交通方式</b>（步行／大眾運輸／開車／<b>✈️ 飛機</b>）與<b>交通時間</b>，時間軸會自動累加計算每站抵達／離開時間，交通段顯示<b>完整通勤時間</b>（出發–抵達）。選「飛機」時可加填<b>航班資訊</b>（例：台灣虎航 IT250 桃園→關西）。</p>
...
${cloudSection}
      ...
      <div class="help-panel__foot">
        <b>最後更新</b>：交通方式新增 ✈️ 飛機（選飛機時可加填航班資訊，如台灣虎航 IT250 桃園→關西，飛機段不做自動路線估算）；交通段改為顯示完整通勤時間（出發–抵達）；桌機拖曳排序改為僅從握把 ⠿ 起拖，圈選景點名稱等文字不再誤觸拖曳（2026-08-06）
      </div>
    </aside>
  `;
}
